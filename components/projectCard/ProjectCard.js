...
import { AddtoBundle, AddtoCart } from "../../utils/api-Request";
      ...
      <div className="pharmaceutical-box">
        ...
        <div className="pharmaceutical-contant">
          ...
          <div className="buttons">
            ...
            <button
              href="#"
              className={isProductExistInBundle(obj.id) ? "addtubul bg-danger":"addtubul"}
              onClick={() => {
                !isProductExistInBundle(obj.id) &&
                  handleAddtoBundle(obj?.id, obj?.type, obj?.quantity);
              }}
              disabled={isProductExistInBundle(obj.id) ? true : false}
            >
              {apicall ? <CircularProgress size={20} /> : <>Add to Bundle</>}
            </button>
          </div>
        </div>
      </div>
